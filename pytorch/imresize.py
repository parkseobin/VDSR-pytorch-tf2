...
def imresize_pil(im, scale_factor=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False):
    '''
        wrapper to handle PIL Image objects

        are im values 0~1 or 0~255?
    '''
    im = np.array(im, dtype=np.float32)
    resized_im = imresize(im, 
        scale_factor=scale_factor, 
        output_shape=output_shape,
        kernel=kernel,
        antialiasing=antialiasing,
        kernel_shift_flag=kernel_shift_flag)
    return Image.fromarray(resized_im.astype(np.uint8))


class ImresizeWrapper(object):
    def __init__(self, scale_factor=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False, vdsr_like=False):
        self.scale_factor = scale_factor
        self.output_shape = output_shape
        self.kernel = kernel
        self.antialiasing = antialiasing
        self.kernel_shift_flag = kernel_shift_flag
        self.vdsr_like = vdsr_like

    def __call__(self, im):
        if(self.vdsr_like):
            downscaled_im = imresize(im, scale_factor=self.scale_factor, kernel='cubic')
            upscaled_im = imresize(downscaled_im, scale_factor=1./self.scale_factor, kernel='cubic')
            return upscaled_im
        else:
            return imresize(im, 
                ...
                kernel_shift_flag=self.kernel_shift_flag)
...
def imresize(im, scale_factor=None, output_shape=None, kernel=None, antialiasing=True, kernel_shift_flag=False):
    ...
    method, kernel_width = {
        ...
        "box": (box, 1.0), # only for downscaling??
        "box2": (box, 6.0), # only for downscaling??
        "linear": (linear, 2.0),
